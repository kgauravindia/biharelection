...
?>
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-6 col-md-4 col-lg-3">
                    ...
                    <ul class="list-unstyled footer-links small mb-0">
                        ...
                        <li class="mb-2"><a href="javascript:void(0)" onclick="openCookieSettings()" class="footer-link">🍪 Cookie Settings</a></li>
    ...
    <div id="cookieConsentBanner" class="cookie-consent-banner" role="region" aria-label="Cookie Consent Banner">
        <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
            <div class="d-flex align-items-start gap-3">
                ...
                <div>
                    <div class="cookie-consent-title">We Value Your Privacy &amp; Consent</div>
                    <p class="cookie-consent-desc">
                        We use cookies and Google services to analyze traffic, enhance site performance, and serve relevant content in accordance with our 
                        <a href="<?php echo SITE_URL; ?>/privacy-policy" target="_blank">Privacy Policy</a>. Choose your preferences below.
                    </p>
                </div>
            </div>
            <div class="cookie-consent-actions mt-2 mt-md-0">
                ...
                <button type="button" class="cookie-btn cookie-btn-reject" onclick="rejectNonEssentialCookies()">
                    Reject Non-Essential
                </button>
                <button type="button" class="cookie-btn cookie-btn-settings" onclick="openCookieSettings()">
    ...
    <button type="button" id="cookieFloatingBtn" class="cookie-settings-trigger" onclick="openCookieSettings()" title="Cookie &amp; Privacy Preferences" aria-label="Cookie Preferences">
    ...
    <div class="modal fade" id="cookieSettingsModal" tabindex="-1" aria-labelledby="cookieSettingsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content rounded-4 border-0 shadow-lg overflow-hidden">
                <div class="modal-header bg-navy text-white px-4 py-3" style="background-color: #0b192c;">
                    <div class="d-flex align-items-center gap-2">
                        <i class="bi bi-shield-lock-fill text-warning fs-4"></i>
                        <h5 class="modal-title fw-bold mb-0 text-white" id="cookieSettingsModalLabel">Cookie &amp; Data Privacy Preferences</h5>
                    </div>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-4">
                    <p class="text-muted small mb-4">
                        When you visit our website, we store and retrieve information on your browser, mostly in the form of cookies. You can manage your preferences below. Learn more in our <a href="<?php echo SITE_URL; ?>/privacy-policy" target="_blank" class="text-primary fw-semibold">Privacy Policy</a>.
                    </p>
                    ...
                    <div class="p-3 bg-light rounded-3 mb-3 border">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <div class="fw-bold text-navy">
                                <i class="bi bi-lock-fill text-secondary me-1"></i> Strictly Necessary Cookies
                            </div>
                            <span class="badge bg-secondary rounded-pill px-2.5 py-1">Always Active</span>
                        </div>
                        <p class="small text-muted mb-0">
                            Essential for core website operation, security authentication, anti-fraud defenses, and citizen portal login sessions.
                        </p>
                    </div>

                    <!-- Option 2: Analytics & Measurement -->
                    <div class="p-3 bg-light rounded-3 mb-3 border">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <label for="consentAnalyticsToggle" class="fw-bold text-navy mb-0 cursor-pointer">
                                <i class="bi bi-graph-up-arrow text-primary me-1"></i> Analytics &amp; Performance Cookies
                            </label>
                            <div class="form-check form-switch mb-0">
                                <input class="form-check-input fs-5" type="checkbox" role="switch" id="consentAnalyticsToggle" checked>
                            </div>
                        </div>
                        <p class="small text-muted mb-0">
                            Allows us to count visits, analyze traffic sources, and identify popular constituency hubs so we can measure and optimize performance (Google Analytics).
                        </p>
                    </div>

                    <!-- Option 3: Advertising & Personalization -->
                    <div class="p-3 bg-light rounded-3 mb-3 border">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <label for="consentAdsToggle" class="fw-bold text-navy mb-0 cursor-pointer">
                                <i class="bi bi-megaphone-fill text-warning me-1"></i> Advertising &amp; Personalization
                            </label>
                            <div class="form-check form-switch mb-0">
                                <input class="form-check-input fs-5" type="checkbox" role="switch" id="consentAdsToggle" checked>
                            </div>
                        </div>
                        <p class="small text-muted mb-0">
                            Used by Google AdSense to personalize ads and provide relevant civic updates based on user interests.
                        </p>
                    </div>
                </div>
                <div class="modal-footer bg-light px-4 py-3 justify-content-between">
                    <button type="button" class="btn btn-outline-secondary rounded-pill fw-semibold px-3 py-2" onclick="rejectNonEssentialCookies(true)">
                        Reject Non-Essential
                    </button>
                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-primary rounded-pill fw-bold px-4 py-2" onclick="saveCustomCookieConsent()">
                            Save Preferences
                        </button>
                        <button type="button" class="btn btn-warning rounded-pill fw-bold text-dark px-4 py-2" onclick="acceptAllCookies(true)">
                            Accept All
                        </button>
                    </div>
                </div>
    ...
    <!-- Google Consent Mode v2 & Cookie Consent Management Script -->
    <script>
    (function() {
        const CONSENT_KEY = 'be_cookie_consent';
        ...
        window.acceptAllCookies = function(fromModal) {
            const consent = { analytics: true, ads: true, timestamp: Date.now() };
            setStoredConsent(consent);
            updateGtagConsent(consent);
            hideBanner();
            if (fromModal && modalEl && window.bootstrap) {
                const modal = bootstrap.Modal.getInstance(modalEl);
                if (modal) modal.hide();
            }
        };

        window.rejectNonEssentialCookies = function(fromModal) {
            const consent = { analytics: false, ads: false, timestamp: Date.now() };
            setStoredConsent(consent);
            updateGtagConsent(consent);
            hideBanner();
            if (fromModal && modalEl && window.bootstrap) {
        ...
        window.saveCustomCookieConsent = function() {
            const consent = {
        ...
        window.openCookieSettings = function() {
            ...
            if (analyticsToggle) {
                analyticsToggle.checked = stored ? Boolean(stored.analytics) : true;
            }
            if (adsToggle) {
                adsToggle.checked = stored ? Boolean(stored.ads) : true;
            }
            if (modalEl && window.bootstrap) {
        ...
        // Show banner on page load if user hasn't made a choice yet
