...
if (isset($_GET['delete_id']) && $conn) {
    ...
    if ($del_id > 0) {
        $stmt = $conn->prepare("DELETE FROM `be_mukhiyas` WHERE `id` = ?");
        if ($stmt) {
...
if ($conn) {
    ...
    $c_res = $conn->query("SELECT COUNT(*) as c FROM `be_mukhiyas` WHERE $where");
    if ($c_res) $total_rows = $c_res->fetch_assoc()['c'];

    $q_res = $conn->query("SELECT * FROM `be_mukhiyas` WHERE $where ORDER BY `district` ASC, `block` ASC, `panchayat` ASC LIMIT $offset, $limit");
    if ($q_res) {
...
<head>
    ...
    <title>Gram Panchayat Mukhiyas — Bihar Election Admin</title>
    ...
    <main class="main-content">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Bihar rural local governance directory of elected Mukhiyas across 38 districts.</p>
            </div>
            <div class="mt-3 mt-md-0">
                <span class="badge bg-primary px-3 py-2 fs-6 rounded-pill">
                    <i class="fas fa-users-gear me-1"></i> <?php echo number_format($total_rows); ?> Mukhiyas Registered
                </span>
            </div>
        </div>
                ...
                <form method="GET" action="mukhiyas.php" class="row g-2 align-items-center">
                    <div class="col-md-5">
                        <div class="input-group">
                            ...
                            <input type="text" name="q" class="form-control border-start-0" placeholder="Search by Mukhiya name, panchayat, block, mobile..." value="<?php echo htmlspecialchars($search); ?>">
                        </div>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="mukhiyas.php" class="btn btn-light border" title="Reset Filters"><i class="fas fa-undo"></i></a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="section-card">
                            ...
                            <?php foreach ($mukhiyas as $m): ?>
                                <tr>
                                    <td>
                                        ...
                                        <?php if (!empty($m['age'])): ?>
                                            <small class="text-muted"><?php echo (int)$m['age']; ?> Yrs</small>
                                        <?php endif; ?>
                                    </td>
                                    <td><span class="text-muted small"><?php echo htmlspecialchars($m['father_husband_name'] ?? '—'); ?></span></td>
                                    ...
                                    <td><span class="fw-bold text-primary"><?php echo htmlspecialchars($m['panchayat']); ?></span></td>
                                    <td>
                                        <div class="small fw-semibold"><?php echo htmlspecialchars($m['category'] ?? 'GEN'); ?></div>
                                    ...
                                    <td class="text-end">
                                        ...
                                        <?php endif; ?>
                                        <a href="mukhiyas.php?delete_id=<?php echo $m['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Delete this Mukhiya record?');" title="Delete">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="8" class="text-center py-5 text-muted">
                                    ...
                                    No Mukhiya records found. Ensure database is connected or seeded via install.php.
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
            ...
            <?php if ($total_pages > 1): ?>
                <div class="p-3 border-top d-flex justify-content-between align-items-center">
                    <small class="text-muted">Showing <?php echo count($mukhiyas); ?> of <?php echo $total_rows; ?> entries</small>
                    <nav>
